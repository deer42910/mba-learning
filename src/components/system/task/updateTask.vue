...
import { editApeTask, getApeTaskById, getUserListByType, getApeMajorList, getCategoryTree } from '../../../api/api';
    ...
    updateId(newVal) {
      if (newVal) {
        getApeTaskById({ id: newVal }).then((res) => {
          if (res.code == 1000) {
            this.form = { ...res.data, id: newVal };
            this.form.state = 2;
            this.form.num = res.data.num + '';
            this.form.proportion = res.data.proportion;
            // 判断 image 是否为外部 URL
            if (res.data.image && res.data.image.startsWith('http')) {
              this.form.image = res.data.image; // 直接使用外部 URL
            } else if (res.data.image) {
              this.form.image = this.$store.state.configure.HOST + res.data.image; // 拼接 HOST
            } else {
              this.form.image = ''; // 如果没有图片，设置为空
            }
            if (this.flag == 1) {
              this.form.type = 0;
            } else {
